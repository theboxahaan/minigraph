...
//  return 0;
  ...
  for(int iter = 0; iter < 1; iter++){
    ...
    for(int i = 0; i < numPoints; i++){
      Dim x1 = rand() % rand_mod + (rand() % 1000)/1000;
      Dim y1 = rand() % rand_mod + (rand() % 1000)/1000;
      input.push_back(std::array<std::pair<Dim, Dim>, 2>({std::pair<Dim, Dim>({x1, (x1 + rand() % dim_diff + (rand() % 1000)/1000) + 1}), 
                                                          std::pair<Dim, Dim>({y1, (y1 + rand() % dim_diff + (rand() % 1000)/1000) + 1})}));
    }
